...
const state = {
  ...
  websiteMode: "submit", // edit only in this variant
  surveyAnswer: "",
  pushVisible: false,
  threadStarted: false,
  threadMessages: [],
...
function renderFeed() {
  const submittedNote = state.feedSubmitted
    ? `
        <div class="survey-brand">UMA Survey</div>
        <div class="survey-tag" style="margin-top:8px;">Submitted to UMA</div>
        <div class="survey-brand" style="margin-top:8px;">
          Your pre-filled details have been sent. We will confirm everything with you in chat.
        </div>
      `
    : '<div class="survey-brand">UMA Survey</div>';

  const buttonLabel = state.feedSubmitting ? "Submitting..." : state.feedSubmitted ? "Submitted" : "Continue";
  const buttonClass = state.feedSubmitted ? "survey-continue ready" : `survey-continue ${state.surveyAnswer ? "ready" : ""}`;
  const buttonDisabled = state.feedSubmitting || state.feedSubmitted || !state.surveyAnswer;
    ...
    <section class="feed-screen">
      ...
      <div class="survey-card">
        ...
        <div class="survey-options">
          <button class="${state.surveyAnswer === "Yes" ? "active" : ""}" type="button" data-action="survey-yes" ${
            state.feedSubmitted ? "disabled" : ""
          }>Yes</button>
          <button class="${state.surveyAnswer === "No" ? "active" : ""}" type="button" data-action="survey-no" ${
            state.feedSubmitted ? "disabled" : ""
          }>No</button>
        </div>
        <button class="${buttonClass}" type="button" data-action="continue-in-feed" ${buttonDisabled ? "disabled" : ""}>${buttonLabel}</button>
      </div>
    </section>
  `;
}
  ...
  return `
    <section class="website-shell">
      <div class="website-top">
        <button type="button" data-action="close-website">Back</button>
        ...
        <div class="uma-page">
          ...
          <div class="uma-brand-lockup">
            ...
            <div class="uma-lockup-copy">
              Please review and update your pre-filled information. Once you confirm it, we will continue the conversation in chat.
            </div>
          </div>
          ...
          <div class="uma-field uma-field-select">
            ...
            <div class="uma-field-control">
              <input id="field-program" value="${escapeAttr(lead.programInterest)}" />
          ...
          <div class="uma-field uma-field-select">
            ...
            <div class="uma-field-control">
              <input id="field-education" value="${escapeAttr(lead.educationLevel)}" />
              <span class="uma-chevron">›</span>
            </div>
          </div>
          ...
          <div class="uma-field">
            ...
            <div class="uma-field-control">
              <input id="field-phone" value="${escapeAttr(lead.phone)}" />
            </div>
          </div>

          <div class="uma-actions">
            <button class="uma-next ${canSubmit ? "ready" : ""}" type="button" data-action="submit-updated-website" ${
              canSubmit ? "" : "disabled"
            }>
              ${state.websiteSubmitting ? "Submitting..." : "Confirm updates"}
            </button>
          </div>
        </div>
      </div>
...
function startThread() {
  ...
  state.threadMessages = [
    ...
    },
    {
      role: "agent",
      html:
        "Hello, what can I help you with? We received your feed submission. Please confirm the information below." +
...
async function submitFeedLead() {
  if (state.feedSubmitting || state.feedSubmitted || !state.surveyAnswer) return;
...
document.addEventListener("click", (event) => {
  ...
  if (action === "survey-yes" && !state.feedSubmitted) {
    state.surveyAnswer = "Yes";
    render();
  }
  if (action === "survey-no" && !state.feedSubmitted) {
    state.surveyAnswer = "No";
    render();
  }
  if (action === "continue-in-feed" && state.surveyAnswer) {
    void submitFeedLead();
  }
